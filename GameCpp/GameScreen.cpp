#include <iostream>
#include "GameScreen.h"

using namespace std;

void GameScreen::FInitScreen()
{
	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(0, 1, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 2, "早             ﹤收收收﹤                   早");
	Screen::getInstance()->ScreenPrint(0, 3, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 4, "早                        ///.   螂~~~      早");
	Screen::getInstance()->ScreenPrint(0, 5, "早                       (^.^)              早");
	Screen::getInstance()->ScreenPrint(0, 6, "早                      曲朽 曳旬           早");
	Screen::getInstance()->ScreenPrint(0, 7, "早                         旨旭             早");
	Screen::getInstance()->ScreenPrint(0, 8, "早                     Ⅸ  旭′             早");
	Screen::getInstance()->ScreenPrint(0, 9, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 10, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 11, "早       螂 埤檣 啪歜  Go! Go!              早");
	Screen::getInstance()->ScreenPrint(0, 12, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 13, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 14, "早       a :豭薹 d : 螃艇薹 space :螂       早");
	Screen::getInstance()->ScreenPrint(0, 15, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 16, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 17, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 18, "早        曲≒旭  space 酈蒂 揚楝輿撮蹂     早");
	Screen::getInstance()->ScreenPrint(0, 19, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 20, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 21, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 22, "曲收收收收收收收收收收收收收收收收收收收收收旭");
}

void GameScreen::FReadyScreen()
{
	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(0, 1, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 2, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 3, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 4, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 5, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 6, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 7, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 8, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 9, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 10, "早﹥﹥﹥﹥﹥                    ﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 11, "早﹥﹥﹥﹥﹥     1  蝶纔檜雖    ﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 12, "早﹥﹥﹥﹥﹥                    ﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 13, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 14, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 15, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 16, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 17, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 18, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 19, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 20, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 21, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 22, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 23, "曲收收收收收收收收收收收收收收收收收收收收收旭");

	
}

void GameScreen::FBackScreen()
{
	char string[100];

	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(0, 1, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 2, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 3, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 4, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 5, "早                                          早");
	sprintf(string, "曳收收收收收收收收收收收收收收收收收收收收收朽薯и 衛除: %d", Stageinfo);
	Screen::getInstance()->ScreenPrint(0, 6, string);
	Screen::getInstance()->ScreenPrint(0, 7, "早                                          早");
	sprintf(string, "早                                          早⑷營 衛除: %d", StageTime);
	Screen::getInstance()->ScreenPrint(0, 8, string);
	Screen::getInstance()->ScreenPrint(0, 9, "早                                          早");
	sprintf(string, "早                                          早跡ル 埤檣: %d ", StageGoal);
	Screen::getInstance()->ScreenPrint(0, 10, string);
	Screen::getInstance()->ScreenPrint(0, 11, "早                                          早");
	sprintf(string, "早                                          早埤檣 奢 偃熱: %d ", StageCount);
	Screen::getInstance()->ScreenPrint(0, 12, string);

	Screen::getInstance()->ScreenPrint(0, 13, "早                                          早");
	sprintf(string, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 14, string);
	Screen::getInstance()->ScreenPrint(0, 15, "早                                          早");
	sprintf(string, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 16, string);
	Screen::getInstance()->ScreenPrint(0, 17, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 18, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 19, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 20, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 21, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 22, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 23, "曲收收收收收收收收收收收收收收收收收收收收收旭");
}

void GameScreen::FGoalMessage(int nX, int nY)
{
	Screen::getInstance()->ScreenPrint(nX, nY, "≧ )) 埤檣 (( ≠");
	Screen::getInstance()->ScreenPrint(nX, nY + 1, "′(^^')/ ′(\"*')/");
	Screen::getInstance()->ScreenPrint(nX, nY + 2, "   ﹥       ﹥");
	Screen::getInstance()->ScreenPrint(nX, nY + 3, "  戎忖    忙戌");
}

void GameScreen::FSuccessScreen()
{
	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(0, 1, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 2, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 3, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 4, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 5, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 6, "早                ////′′                  早");
	Screen::getInstance()->ScreenPrint(0, 7, "早               q ∥  ∥ p                 早");
	Screen::getInstance()->ScreenPrint(0, 8, "早               (戌式式戎)                 早");
	Screen::getInstance()->ScreenPrint(0, 9, "早             Ｏ 嘐暮 撩奢 Ｎ              早");
	Screen::getInstance()->ScreenPrint(0, 10, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 11, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 12, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 13, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 14, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 15, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 16, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 17, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 18, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 19, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 20, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 21, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 22, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 23, "曲收收收收收收收收收收收收收收收收收收收收收旭");
}

void GameScreen::FFailureScreen()
{
	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(0, 1, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 2, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 3, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 4, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 5, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 6, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 7, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 8, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 9, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 10, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 11, "早                    嘐暮 褒ぬ !!!!        早");
	Screen::getInstance()->ScreenPrint(0, 12, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 13, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 14, "早                 ≒有收旬                 早");
	Screen::getInstance()->ScreenPrint(0, 15, "早                   旭  曲                 早");
	Screen::getInstance()->ScreenPrint(0, 16, "早                  ﹥﹥﹥﹥                早");
	Screen::getInstance()->ScreenPrint(0, 17, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 18, "早        棻衛 ж衛啊蝗棲梱? (y/n)          早");
	Screen::getInstance()->ScreenPrint(0, 19, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 20, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 21, "早                                          早");
	Screen::getInstance()->ScreenPrint(0, 22, "曲收收收收收收收收收收收收收收收收收收收收收旭");
}

void GameScreen::FTimeAttack()
{
	if (time->DelayCheck())
	{
		StageTime++;
	}
}

void GameScreen::FGoal()
{
	StageCount++;
}

bool GameScreen::FFailed()
{
	if (Stageinfo == StageTime)
	{
		return true;
	}
}

bool GameScreen::FClear()
{
	if (StageGoal == StageCount)
		return true;
}

void GameScreen::GameSelect()
{
	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(0, 1, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 2, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 3, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 4, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 5, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 6, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 7, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 8, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 9, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 10, "早﹥﹥﹥﹥﹥                        ﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 12, "早﹥﹥﹥﹥﹥1: Е獐啪歜 2: 戀觼啪歜 ﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 13, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 14, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 15, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 16, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 17, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 18, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 19, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 20, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 21, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 22, "早﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥﹥早");
	Screen::getInstance()->ScreenPrint(0, 23, "曲收收收收收收收收收收收收收收收收收收收收收收收旭");
}

GameScreen::GameScreen()
{
	Stageinfo = 30;
	StageTime = 0;
	StageGoal = 6;
	StageCount = 0;
	time = new Time(1000);
}

GameScreen::~GameScreen()
{
}

void GameScreen::TInitScreen()
{
	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(0, 1, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 2, "早         -----                                          早");
	Screen::getInstance()->ScreenPrint(0, 3, "早        /    |                                       /  早");
	Screen::getInstance()->ScreenPrint(0, 4, "早       /       |    戀觼 啪歜                       /   早");
	Screen::getInstance()->ScreenPrint(0, 5, "早      /          |                            /-----/   早");
	Screen::getInstance()->ScreenPrint(0, 6, "早     /            |---------------|         /           早");
	Screen::getInstance()->ScreenPrint(0, 7, "早    /                               |      /            早");
	Screen::getInstance()->ScreenPrint(0, 8, "早   /              旨收收收收收旬      |   /             早");
	Screen::getInstance()->ScreenPrint(0, 9, "早  /               早          早       |/               早");
	Screen::getInstance()->ScreenPrint(0, 10, "早/                 早    ≡    早                        早");
	Screen::getInstance()->ScreenPrint(0, 11, "早     ( (    旨收收收收收收收收收收收旬     ) )          早");
	Screen::getInstance()->ScreenPrint(0, 12, "早            早                      早                  早");
	Screen::getInstance()->ScreenPrint(0, 13, "早           〥 〥                   〥 〥                早");
	Screen::getInstance()->ScreenPrint(0, 14, "早           〥 〥 收收收收收收收收收〥 〥                早");
	Screen::getInstance()->ScreenPrint(0, 15, "早           〥 〥                   〥 〥                早");
	Screen::getInstance()->ScreenPrint(0, 16, "早  ~ ^ ~ $~ ~ ^ ~ ~ ~ ^ ~ ~ ~ ^ ~ ~ ~ ^~ ~ ~ ~ ~ $ ~    ~早");
	Screen::getInstance()->ScreenPrint(0, 17, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 18, "早                  Press Space Key !!                    早");
	Screen::getInstance()->ScreenPrint(0, 19, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 20, "曲收收收收收收收收收收收收收收收收收收收收收收收收收收收收旭");
}

void GameScreen::TBox()
{
	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收收收收收收收收旬");
	for (int i = 1; i <= 19; i++)
		Screen::getInstance()->ScreenPrint(0, i, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 20, "曲收收收收收收收收收收收收收收收收收收收收收收收收收收收收旭");
}

void GameScreen::TReadyScreen()
{
	char string[100] = "1 蝶纔檜雖 !!!!!!";
	TBox();
	Screen::getInstance()->ScreenPrint(25, 9, string);
}

void GameScreen::TSucceesScreen()
{
	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(0, 1, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 2, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 3, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 4, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 5, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 6, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 7, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 8, "早                     ////′′                           早");
	Screen::getInstance()->ScreenPrint(0, 9, "早                    q ∥. ∥ p (^)@                     早");
	Screen::getInstance()->ScreenPrint(0, 10, "早                    (戌式式戎) //                       早");
	Screen::getInstance()->ScreenPrint(0, 11, "早                   Ｏ 嘐暮 撩奢 Ｎ                      早");
	Screen::getInstance()->ScreenPrint(0, 12, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 13, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 14, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 15, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 16, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 17, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 18, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 19, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 20, "曲收收收收收收收收收收收收收收收收收收收收收收收收收收收收旭");
}

void GameScreen::TFailScreen()
{
	Screen::getInstance()->ScreenPrint(0, 0, "旨收收收收收收收收收收收收收收收收收收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(0, 1, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 2, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 3, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 4, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 5, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 6, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 7, "早                    嘐暮 褒ぬ !!!!                      早");
	Screen::getInstance()->ScreenPrint(0, 8, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 9, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 10, "早                               ≒有收旬                 早");
	Screen::getInstance()->ScreenPrint(0, 11, "早                                 旭  曲                 早");
	Screen::getInstance()->ScreenPrint(0, 12, "早                                ﹥﹥﹥﹥                早");
	Screen::getInstance()->ScreenPrint(0, 13, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 14, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 15, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 16, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 17, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 18, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 19, "早                                                        早");
	Screen::getInstance()->ScreenPrint(0, 20, "曲收收收收收收收收收收收收收收收收收收收收收收收收收收收收旭");
}

void GameScreen::TStopScreen()
{

	TBox();

	Screen::getInstance()->ScreenPrint(3, 3, "                     戀觼 啪歜 部");
	Screen::getInstance()->ScreenPrint(3, 6, "                    旨收收收收收旬");
	Screen::getInstance()->ScreenPrint(3, 7, "               ~~   早          早");
	Screen::getInstance()->ScreenPrint(3, 8, "        ~~          早    ≡    早");
	Screen::getInstance()->ScreenPrint(3, 9, "    ≒         旨收收收收收收收收收收收旬");
	Screen::getInstance()->ScreenPrint(3, 10, "  曲早旭       早                      早");
	Screen::getInstance()->ScreenPrint(3, 11, "   旨旬      〥 〥                   〥 〥");
	Screen::getInstance()->ScreenPrint(3, 12, "             〥 〥 收收收收收收收收收〥 〥");
	Screen::getInstance()->ScreenPrint(3, 13, "             〥 〥                   〥 〥");
	Screen::getInstance()->ScreenPrint(3, 14, "~ ~ ^ ~ $~ ~ ^ ~ ~ ~ ^ ~ ~ ~ ^ ~ ~ ~ ^~ ~ ~ ~ /~ $ ~~");
}
